...
## Steps
1. Write `data/<yyyy-mm-dd>-<topic>-<speaker>.json` (slug = same string, e.g. `2026-10-15-agent-security-ke-he`) following `data/schema.json` (copy `data/example.json`).
2. Run `npm run build data/<file>.json` — it installs anything missing on first use, renders both posters, validates, and builds the hand-off pack. Don't run npm install / playwright install yourself.
...
## Don'ts
- Don't change `templates/` or `specs/` for a single event.
